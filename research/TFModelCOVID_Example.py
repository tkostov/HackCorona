...
TPop = np.sum(PopTotalLK)  # total population
...
if True:
    ...
    else:  # each LK has its own start!
        individualStart = np.ones(LKPopulation.shape, covid.CalcFloatStr) * StartI0 / NumLK / NumAge
        I0Var = tf.Variable(initial_value=individualStart, dtype=covid.CalcFloatStr, name='I0Var')  # start with one in every age group and district
else:
    I0Var = StartI0
...
SimTimes = LKReported.shape[0] + 10
...
StartRateToHospital = 0.054
if False:  # global fit for infections
    TotalRateToHospital = StartRateToHospital
else:
    TotalRateToHospital = tf.Variable(initial_value=StartRateToHospital, name='TotalRateToHospital', dtype=covid.CalcFloatStr)  # float(0.15/TPop)
...
allVars = allVars + [I0Var, InfectionRateTotal, TotalRateToHospital]
...
# Important! Because of the normalization of the states, we have to multiply with the TOTAL Population,
# not the population distribution!
reported, hospitalized, cured, dead = covid.measuredStates(allRes, TPop, byAge=True)
...
print('----Fitting --- Smmary')
offs = len(allRes)
for n in np.arange(len(allVars)):
    print(allVars[n].name + ' is ' + str(np.mean(res[offs + n])))
